...
const POSPage: React.FC = () => {
  ...
  const { data: clientes = [] } = useQuery<Cliente[]>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${API_URL}/api/clientes?search=${encodeURIComponent(clienteSearch)}`,
      );
  ...
  const filteredProducts = productos.filter((producto) => {
    ...
    const matchesCategory =
      !activeCategory || producto.categoria?.cat_nombre === activeCategory;
    ...
    <MainLayout>
      <div className="flex h-[calc(100vh-64px)]">
        ...
        <div className="flex-1 overflow-auto bg-gray-50 p-4">
              ...
              >
                <CardContent className="flex flex-col items-center justify-center p-2 text-center">
                  ...
                  <div className="text-xs text-gray-500">
                    {
                      productos.filter(
                        (p) => p.categoria?.cat_nombre === cat.cat_nombre,
                      ).length
                    }{" "}
                    items
                  </div>
          ...
          <div
            className="grid grid-cols-5 gap-3 max-h-[calc(100vh-300px)] overflow-y-auto"
            style={{ gridTemplateRows: "repeat(2, 1fr)" }}
          >
            {!loadingProducts &&
              filteredProducts.slice(0, 10).map((producto) => {
                const cartItem = cartItems.find(
                  (item) => item.id === parseInt(producto.pro_id),
                );
                return (
                  <Card
                    key={producto.pro_id}
                        ...
                        {cartItem ? (
                          <div className="flex items-center justify-center border rounded bg-primary bg-opacity-10 border-primary p-0.5">
                            ...
                            <button
                              className="p-1 text-primary hover:bg-primary hover:bg-opacity-20 rounded transition-colors"
                              disabled={
                                (producto.pro_stock || 0) <= cartItem.quantity
                              }
                              onClick={() =>
                                updatePOSQuantity(
                                  producto.pro_id,
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between">
                          ...
                          <button
                            onClick={() =>
                              removeFromPOSCart(item.id.toString())
                            }
                          >
        ...
        <DialogContent className="sm:max-w-5xl max-h-[95vh] overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm">
              Gestiona la información del pedido y administra los productos del
              carrito
            </DialogDescription>
          </DialogHeader>
                      ...
                      {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item) => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className="flex items-center gap-3 bg-gray-50 border rounded-lg p-3 hover:shadow-sm transition-shadow"
                            >
                    ...
                    <div className="text-sm space-y-2">
                      ...
                      {notas && (
                        <div className="pt-2 border-t">
                          ...
                          <p className="text-xs mt-1 bg-gray-50 p-2 rounded italic line-clamp-2">
                            {notas}
                          </p>
                        </div>
                      )}
